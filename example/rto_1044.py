...
# Preconditions:
# - Installed RsInstrument Python module Version 1.2.0.25 or newer from pypi.org
...
rto = None
try:
	rto = RsInstrument('GPIB::20::INSTR', True, False)
...
# -----------------------------------------------------------
rto.VisaTimeout = 200000  # Acquisition timeout - set it higher than the acquisition time
rto.write_str("SING")
# -----------------------------------------------------------
...
rto.write_str(r"MMEM:NAME 'c:/temp/Dev_Screenshot.png'")  # Set the screenshot path
rto.write_str("HCOP:IMM")  # Make the screenshot now
rto.query_opc()  # Wait for the screenshot to be saved
rto.read_file_from_instrument_to_pc(r'c:/temp/Dev_Screenshot.png', r'test2.png')  # Query the instrument file to the PC
print(r"Screenshot file saved to PC 'test2.png'")

# -----------------------------------------------------------
# Exporting Interleaved x/y Data of a Single Waveform to CSV File
# -----------------------------------------------------------

rto.write_str('EXPort:WAVeform:FASTexport ON')
rto.write_str('CHANnel1:WAVeform1:STATe 1')

rto.query_opc()
rto.write_str('EXPort:WAVeform:SOURce C1W1')
rto.write_str('EXPort:WAVeform:SCOPe WFM')
rto.write_str(r"EXPort:WAVeform:NAME 'c:\temp\akiyo_DataExportWfm_analog.csv'")

rto.write_str('EXPort:WAVeform:RAW ON')
rto.write_str('EXPort:WAVeform:INCXvalues ON')
rto.write_str('EXPort:WAVeform:DLOGging OFF')
rto.write_str('EXPort:WAVeform:SAVE')
rto.query_opc()
rto.read_file_from_instrument_to_pc(r'c:\temp\akiyo_DataExportWfm_analog.Wfm.csv', r'akiyo4.csv')
